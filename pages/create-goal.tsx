...
export default function CreateGoal(props:any){
  ...
  const handleSubmit = (e:any)=>{
    ...
    const id = 'g'+Date.now();
    goals.unshift({id,title,desc,deadline,stake: Number(stake),useValidator,fee,total,status:'active',progress:0});
        ...
        <div className="card" style={{marginTop:20}}>
          ...
          <form onSubmit={handleSubmit} style={{marginTop:12,display:'grid',gap:12}}>
            ...
            <div style={{display:'flex',gap:8}}>
              <button className="btn" type="submit">Create & Pay (placeholder)</button>
              <button type="button" className="btn ghost" onClick={()=>localStorage.removeItem('smg_goals') || alert('Cleared demo goals')}>Clear Demo Goals</button>
            </div>
          </form>
        </div>
      </main>
