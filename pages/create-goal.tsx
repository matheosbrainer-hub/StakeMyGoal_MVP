...
export default function CreateGoal(props:any){
  ...
  const handleSubmit = (e:any)=>{
    ...
    if (useValidator && validatorEmail) {
      // simulate invitation
      const invites = JSON.parse(localStorage.getItem('smg_invites')||'[]');
      invites.unshift({id:'i'+Date.now(), email:validatorEmail, goalId:id, time:Date.now()});
      localStorage.setItem('smg_invites', JSON.stringify(invites));
      alert('✅ Invitation sent to ' + validatorEmail);
    }
