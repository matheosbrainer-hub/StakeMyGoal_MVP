...
export default function GoalCard({ goal, onDelete, onUpdate }: any){
  ...
  const calcTimeProgress = (g:any)=>{
    try {
      const created = g.createdAt ? new Date(g.createdAt).getTime() : Date.now();
      const start = created;
  ...
  return (
    <div className="goal card">
      ...
      <div style={{marginTop:8,fontSize:13}} className="muted">Deadline: {local.deadline || '—'} • {local.useValidator? 'Validator':'Solo'} {local.validatorEmail? '• '+local.validatorEmail : ''}</div>
      <div className="progress" style={{marginTop:10}}><i style={{width:`${timePct}%`, background:progressColor}}></i></div>
      ...
      {showEdit && <div style={{marginTop:8}}>
        ...
        <input className="input" type="date" value={local.deadline} onChange={e=>setLocal({...local,deadline:e.target.value})} />
